...
def main():
    ...
    parser.add_argument("--full",         default=False, required=False, help="Set to True to train on all data (train + val + test) but metrics will not be representative as there will be data leakage. Default is False.")
    parser.add_argument("--debugging",      default="True", required=False, help="Set to True to enable debugging mode with verbose logging and reduced dataset size for faster iteration.")
                        
    
    args = parser.parse_args()
    full_training = args.full.lower() == "true" if args.full else False
    debugging_mode = args.debugging.lower() == "true" if args.debugging else False
    ...
    # Load best params if available
        ...
        if debugging_mode:
            print("Debugging mode enabled. Reducing dataset size for faster iteration.")
            # ── Train ──────────────────────────────────────────────────────────
            ...
            print("="*65)
            print("  Model is saved locally")
            print("  Artifacts (plots, model) are stored on locally.")
            print("="*65)
